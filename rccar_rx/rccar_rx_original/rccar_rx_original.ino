...
void loop() {
  ...
  // If joystick stays in the middle, the motors are not moving
  else {
    motorSpeedA = 0;
    motorSpeedB = 0;
  }
  ...
  if (xAxis < 470) {
    ...
    // Move to the left - decrease left motor speed, increase right motor speed
  ...
  if (xAxis > 550) {
    ...
    // Move to the right - decrease right motor speed, increase left motor speed
    motorSpeedA = motorSpeedA + xMapped;
    motorSpeedB = motorSpeedB - xMapped;
    // Confine the range from 0 to 255
    if (motorSpeedA > 255) {
  ...
  // Prevent buzzing at low speeds (Adjust according to your motors. My motors couldn't start moving if the PWM value was below the value of 70)
  if (motorSpeedA < 70) {
    motorSpeedA = 0;
  }
  if (motorSpeedB < 70) {
